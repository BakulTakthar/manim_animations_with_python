...
class MainScene(ThreeDScene):
    def construct(self):
        ...
        l1_label = MathTex('l_{1} : r_{1}=a_{1}+\lambda d_{1}', color = BLACK).move_to(line1)
        ...
        l2_label = MathTex('l_{2} : r_{2}=a_{2}+\lambda d_{2}', color = BLACK).move_to(line2)
        ...
        x_label = MathTex("x").move_to(axes.c2p(5, 0, 0) + np.array([0.3, 0, 0])).rotate(3*PI/2,LEFT)
        y_label = MathTex("y").move_to(axes.c2p(0, 5, 0) + np.array([0, 0.3, 0])).rotate(3*PI/2,LEFT)
        z_label = MathTex("z").move_to(axes.c2p(0, 0, 5) + np.array([0, 0, 0.3])).rotate(3*PI/2,LEFT)
        l1_label.move_to(line1)

        # Add everything to the scene
        self.add(x_y_grid, y_z_grid, x_z_grid, axes, x_label.set_color(BLACK), y_label.set_color(BLACK), z_label.set_color(BLACK), line1, line2, line3)
